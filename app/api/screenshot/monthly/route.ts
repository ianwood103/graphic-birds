// /pages/api/create-zip.ts (or your desired API route path)
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const graphics = [
    "monthlybirdtotal",
    "monthlyspeciesbreakdown",
    "monthlymapview",
    "monthlybirdtotalv2",
    "monthlyspeciesbreakdownv2",
    "monthlymapviewv2",
    "monthlybanner",
  ];

  const requestBody = await request.json();
  const { id, month, year } = requestBody;

  const objectKeys = await Promise.all(
    graphics.map(async (graphic) => {
      const requestBody = {
        url: `${process.env.NEXT_PUBLIC_URL}/${graphic}/${id}?month=${month}&year=${year}`,
        selector: `#${graphic}`,
      };
